...
function updateUserRank($conn, $user_id, $order_date = null) {
    try {
        // 사용자 정보 가져오기
        $stmt = $conn->prepare("
            SELECT rank, myQuantity, myTotal_quantity, myAgent, myAgent_referral, referred_by
            FROM users WHERE id = ?
        ");
       ...
       // 총판 승급 조건 확인(본인실적)
        if ($user['myQuantity'] >= 1000 && $current_rank == '회원') {
            $new_rank = '총판';
        }
    ...
    try {
        ...
        while ($user_id) {
            ...
            if ($user['rank'] == '총판') {
                ...
                $commission_amount = $orderAmount * 0.20; // 20%
                break;
        ...
        while ($user_id) {
            ...
            if ($user['rank'] == '특판' || $user['rank'] == '특판A') {
                ...
                if ($original_user_rank == '회원') {
                    if ($has_distributor) {
                        $commission_amount = $orderAmount * 0.05; // 총판이 있는 경우 5%
                    } else {
                        $commission_amount = $orderAmount * 0.25; // 총판이 없는 경우 25%
                    }
                } elseif ($original_user_rank == '총판') {
                    $commission_amount = $orderAmount * 0.05; // 총판 위의 특판은 항상 5%
                }
                break;
            }
